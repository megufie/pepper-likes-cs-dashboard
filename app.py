...
PAGE_LABELS = ["概要", "継続率分析", "解約分析",
               "募集品質KPI", "応募分析", "企業レポート", "未回収債権", "CS業務用", "利用状況"]
PAGE_KEYS   = ["summary", "retention", "churn",
               "quality", "applications", "company", "uncollected", "cs_ops", "usage"]
...
def render_job_quality():
    page_header("募集品質KPI",
                "魅力スコア（0–12点・重み付け）と募集運用健全度を分析")

    postings = analytics.compute_posting_quality(con)
    if postings.empty:
        st.info("データがありません。")
        return

    rank_counts = postings["ランク"].value_counts()
    pill_styles = {
        "S": f"background:{MINT_BG};color:{MINT_DARK};border:1.5px solid {MINT};",
        "A": f"background:#E5F0FB;color:#1E5A99;border:1.5px solid #5AA0DC;",
        "B": "background:#FEF6E5;color:#7A4A00;border:1.5px solid #F5A623;",
        "C": "background:#FFE5D6;color:#7A3A00;border:1.5px solid #FF8C42;",
        "D": "background:#FBECEC;color:#7A1E1E;border:1.5px solid #D9534F;",
    }
    pills = "".join(
        f'<span class="rank-pill" style="{pill_styles[r]}">'
        f'{r} ランク {rank_counts.get(r, 0)}件</span>'
        for r in ["S", "A", "B", "C", "D"]
    )
    st.markdown(pills, unsafe_allow_html=True)
    st.markdown('<div style="height:14px"></div>', unsafe_allow_html=True)

    # ── Score distribution + age distribution ────────────────────────────────
    col1, col2 = st.columns(2)
    with col1:
        with st.container(border=True):
            section("スコア分布（0–12点）",
                    "S=9–12点 / A=7–8 / B=5–6 / C=3–4 / D=0–2")
            fig = px.histogram(
                postings, x="attractiveness_score", color="ランク",
                color_discrete_map={**RANK_COLORS, "D": "#D9534F"},
                nbins=13,
                labels={"attractiveness_score": "魅力スコア", "count": "件数"},
    ...
    with col2:
        with st.container(border=True):
            section("📅 募集年齢分布（アクティブ募集のみ）",
                    "横軸：作成からの経過日数 ／ 30日サイクルが理想")
            ages = queries.get_posting_age_distribution(con)
            if not ages.empty:
                ages = ages.copy()
                ages["label"] = ages["age_bucket"].str.replace(r"^\d:\s*", "", regex=True)
                colors = ["#4FB89A", "#7CC2A9", "#A8DCC9",
                          "#F5A623", "#E8772E", "#D9534F"]
                fig = px.bar(ages, x="label", y="posting_count",
                             text="posting_count",
                             labels={"label": "経過日数", "posting_count": "件数"})
                fig.update_traces(marker_color=colors[:len(ages)],
                                  marker_line_width=0,
                ...
                themed(fig, margin=dict(l=0, r=0, t=20, b=0))
                st.plotly_chart(fig, use_container_width=True, config=CHART_CFG)

    # ── Score×adoption + age×adoption ────────────────────────────────────────
    col3, col4 = st.columns(2)
    with col3:
        with st.container(border=True):
            section("スコア × 採用率",
                    "スコアが高くても採用率が低い募集はCS介入候補")
            scatter_df = postings.dropna(subset=["adoption_rate_pct"])
            if not scatter_df.empty:
                fig2 = px.scatter(
                    scatter_df, x="attractiveness_score", y="adoption_rate_pct",
                    color="ランク",
                    color_discrete_map={**RANK_COLORS, "D": "#D9534F"},
                    hover_data=["company_name", "title"],
                    labels={"attractiveness_score": "魅力スコア",
                            "adoption_rate_pct":    "採用率（%）"},
                    category_orders={"ランク": ["S", "A", "B", "C", "D"]},
                )
                ...
                st.info("応募3件以上の募集がないため表示できません。")

    with col4:
        with st.container(border=True):
            section("⏰ 募集年齢 × 採用率",
                    "古い募集ほど採用率が下がるか? — 差替えサイクルの根拠")
            age_corr = queries.get_age_vs_adoption_correlation(con)
            if not age_corr.empty:
                fig3 = px.scatter(
                    age_corr, x="age_days", y="adoption_rate_pct",
                    hover_data=["title"],
                    labels={"age_days": "募集年齢（日）",
                            "adoption_rate_pct": "採用率（%）"},
                )
                fig3.update_traces(
                    marker=dict(size=8, color=MINT, opacity=0.7),
                )
    ...
    with st.container(border=True):
        section("🔄 企業別 募集差替え活動",
                "直近30日の新規募集 + 90日超の古い募集 — CSフォロー対象を特定")
        refresh = queries.get_company_refresh_activity(con)
        if not refresh.empty:
            disp = refresh.head(30).rename(columns={
                "company_name":        "企業名",
                "total_postings":      "総募集数",
                "active_postings":     "アクティブ",
                "new_in_30d":          "直近30日 新規",
                "stale_active":        "90日超 (古い)",
                "latest_posting_date": "最新作成日",
            ...
            def _new(v):
                ...
                if n == 0: return "color:#CCC;text-align:center;"
                return (f"background-color:rgba(79,184,154,0.4);color:{MINT_DARK};"
                        "font-weight:700;text-align:center;border-radius:8px;")
            ...
            def _stale(v):
                ...
                if n == 0: return "color:#CCC;text-align:center;"
                return ("background-color:rgba(217,83,79,0.2);color:#7A1E1E;"
                        "font-weight:700;text-align:center;border-radius:8px;")

            styled = (disp.style
                .map(_new, subset=["直近30日 新規"])
                .map(_stale, subset=["90日超 (古い)"])
                .set_properties(**{"font-size": "12px"}))
    ...
    with st.container(border=True):
        section("📋 全募集 一覧（スコア順）")
        cols = ["company_name", "title", "attractiveness_score", "ランク",
                "image_count", "follower_count_id", "total_applications",
                "adoption_rate_pct", "posting_age_days", "status"]
        avail = [c for c in cols if c in postings.columns]

        def _rank(v):
            bg = {"S": MINT_BG, "A": "#E5F0FB", "B": "#FEF6E5",
                  "C": "#FFE5D6", "D": "#FBECEC"}
            return f"background-color:{bg.get(v,'')};font-weight:700;text-align:center;"

        def _score(v):
            try: a = int(v) / 12
            except: return ""
            return (f"background-color:rgba(79,184,154,{0.10 + a*0.50:.2f});"
                    f"color:{MINT_DARK};font-weight:700;text-align:center;")

        def _age(v):
            try: n = int(v)
            except: return ""
            if n <= 30:  return f"background-color:{MINT_BG};color:{MINT_DARK};text-align:center;"
            if n <= 90:  return "background-color:#FEF6E5;color:#7A4A00;text-align:center;"
            return "background-color:#FBECEC;color:#7A1E1E;text-align:center;"

        rename_map = {
            "company_name": "企業名", "title": "募集タイトル",
            "attractiveness_score": "スコア",
            "image_count": "画像数",
            "follower_count_id": "Fr要件",
            "total_applications": "応募数",
            "adoption_rate_pct": "採用率(%)",
            "posting_age_days": "経過日",
            "status": "ステータス",
        }
        disp = postings[avail].rename(columns=rename_map)
        styler = disp.style.map(_rank, subset=["ランク"]).map(_score, subset=["スコア"])
        if "経過日" in disp.columns:
            styler = styler.map(_age, subset=["経過日"])
        st.dataframe(styler.set_properties(**{"font-size": "12px"}),
                     use_container_width=True, height=420)


# ── Page: 企業レポート (Company Detail) ───────────────────────────────────────

def render_company_detail():
    page_header("企業レポート", "企業ごとの活動・募集・継続状況をドリルダウン")

    companies_df = con.execute(
    ...
    selected = st.selectbox("企業を選択", companies_df["company_name"].tolist(),
                            label_visibility="collapsed")
    cid = companies_df.loc[
        companies_df["company_name"] == selected, "company_id"
    ].iloc[0]
    detail = analytics.get_company_detail_summary(con, cid)
    info   = detail["company_info"]

    if not info.empty:
        row = info.iloc[0]
        status = row.get("status", "-")
        s_color = MINT_DARK if status == "active" else "#D9534F"
        s_bg    = MINT_BG    if status == "active" else "#FBECEC"
        start = str(row.get("contract_start_date", "-"))[:10]

        st.markdown(f"""
        <div class="kpi" style="margin-bottom:14px;display:flex;gap:36px;align-items:center;">
            <div>
                <div class="kpi-label">ステータス</div>
                <span class="status-badge" style="background:{s_bg};color:{s_color};">{status}</span>
            </div>
            <div>
                <div class="kpi-label">プラン</div>
                <div style="font-size:14px;font-weight:600;color:{INK}">{row.get('plan_type','-')}</div>
            </div>
            <div>
                <div class="kpi-label">担当営業</div>
                <div style="font-size:14px;font-weight:600;color:{INK}">{row.get('cs_owner','-')}</div>
            </div>
            <div>
                <div class="kpi-label">契約開始日</div>
                <div style="font-size:14px;font-weight:600;color:{INK}">{start}</div>
            </div>
            <div style="margin-left:auto;text-align:right;">
                <div class="kpi-label">継続月数</div>
                <div style="font-size:28px;font-weight:700;color:{MINT_DARK};
                            letter-spacing:-0.8px;line-height:1;">{detail['active_months']}</div>
            </div>
            <div style="text-align:right;">
                <div class="kpi-label">平均魅力スコア</div>
                <div style="font-size:28px;font-weight:700;color:{INK};
                            letter-spacing:-0.8px;line-height:1;">{detail['avg_score']:.1f}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        with st.container(border=True):
            section("週次メッセージ送信数（直近12週）")
            weekly = detail["weekly_messages"]
            if weekly.empty:
                st.info("直近12週にデータがありません。")
            else:
                fig = px.bar(weekly, x="iso_week", y="message_count",
                             labels={"iso_week": "週", "message_count": "送信数"})
                fig.update_traces(marker_color=MINT, marker_line_width=0,
                                  marker_cornerradius=4)
    ...
    with col2:
        with st.container(border=True):
            ...
            if pdf.empty:
                ...
                fig2 = px.bar(
                    pdf.sort_values("attractiveness_score"),
                    x="attractiveness_score", y="title",
                    color="ランク", color_discrete_map=RANK_COLORS,
                    orientation="h",
                    labels={"attractiveness_score": "スコア", "title": ""},
                    category_orders={"ランク": ["S", "A", "B", "C"]},
                )
                fig2.update_layout(yaxis_tickfont_size=10, showlegend=True,
                                   legend=dict(orientation="h", y=1.12, x=0),
                                   height=260)
                themed(fig2)
                st.plotly_chart(fig2, use_container_width=True, config=CHART_CFG)
    ...
    with st.container(border=True):
        ...
        pdf = detail["postings"]
        if not pdf.empty:
            cols = ["title", "attractiveness_score", "ランク", "status",
                    "image_count", "description_length", "compensation_type",
                    "has_deadline", "has_sample"]
            avail = [c for c in cols if c in pdf.columns]

            def _rank(v):
                bg = {"S": MINT_BG, "A": "#E5F0FB", "B": "#FEF6E5", "C": "#FBECEC"}
                return f"background-color:{bg.get(v,'')};font-weight:700;text-align:center;"

            st.dataframe(
                pdf[avail].rename(columns={
                    "title": "タイトル", "attractiveness_score": "スコア",
                    "status": "ステータス", "image_count": "画像枚数",
                    "description_length": "説明文字数", "compensation_type": "報酬形態",
                    "has_deadline": "締切", "has_sample": "サンプル",
                }).style.map(_rank, subset=["ランク"])
                  .set_properties(**{"font-size": "12px"}),
...

{
    ...
    "churn":        render_churn,
    "quality":      render_job_quality,
    "applications": render_applications,
    "company":      render_company_detail,
